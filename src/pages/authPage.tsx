  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-slate-50 to-slate-100 dark:from-slate-950 dark:to-slate-900">
      <div className="mx-auto flex min-h-screen max-w-md items-center justify-center px-4 py-12">
        <div className="w-full rounded-3xl bg-white p-6 shadow-xl ring-1 ring-slate-200 dark:bg-slate-900 dark:ring-slate-800 sm:p-8">
          ...
          <form onSubmit={onSubmit} className="mt-6 space-y-4">
            ...
            <button
              ...
              className={cx(
                ...
                loading || !canSubmit
                  ? "cursor-not-allowed bg-slate-200 text-slate-500 ring-slate-200 dark:bg-slate-800 dark:text-slate-500 dark:ring-slate-700"
                  : "bg-slate-900 text-white ring-slate-900 hover:bg-slate-800 dark:bg-slate-100 dark:text-slate-900 dark:ring-slate-100 dark:hover:bg-white"
              )}
            >
            ...
            <div className="pt-2 text-center text-sm text-slate-600 dark:text-slate-400">
              {mode === "login" && (
                <>
                  Non hai un account?{" "}
                  <button type="button" onClick={() => setMode("register")} className="font-semibold text-slate-900 hover:underline dark:text-slate-100">
                    Registrati
                  </button>
                </>
              )}
              ...
              {mode === "register" && (
                <>
                  Hai già un account?{" "}
                  <button type="button" onClick={() => setMode("login")} className="font-semibold text-slate-900 hover:underline dark:text-slate-100">
                    Accedi
                  </button>
                </>
              )}
          ...
          <div className="mt-8 rounded-2xl bg-slate-50 px-4 py-3 text-xs text-slate-600 ring-1 ring-slate-200 dark:bg-slate-800 dark:text-slate-400 dark:ring-slate-700">
            {mode === "join" ? (
              <span>
...
function Field(props: {
  ...
  return (
    <div>
      <label className="block text-sm font-semibold text-slate-800 dark:text-slate-200">{props.label}</label>
      <input
        ...
        className={cx(
          ...
          props.disabled && "bg-slate-50 text-slate-500 dark:bg-slate-800 dark:text-slate-500"
        )}
      />
      {props.hint && <div className="mt-2 text-xs text-slate-500 dark:text-slate-400">{props.hint}</div>}
    </div>
  );
}
